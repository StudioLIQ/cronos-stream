...
import { generateStreamRecap } from '../agent/recapCopilot.js';
...
// POST /api/channels/:slug/recap - Generate a post-stream recap (dashboard auth, local Ollama)
router.post('/channels/:slug/recap', async (req, res, next) => {
  try {
    const { slug } = req.params;
    ...
    if (fromRaw || toRaw) {
      const parsedFrom = fromRaw ? parseDate(fromRaw) : null;
      const parsedTo = toRaw ? parseDate(toRaw) : null;
      if ((fromRaw && !parsedFrom) || (toRaw && !parsedTo)) {
        res.status(400).json({ error: 'Invalid from/to datetime (use ISO8601)' });
        return;
      }
      from = parsedFrom || new Date(now.getTime() - 24 * 60 * 60 * 1000);
      to = parsedTo || now;
    ...
    if (from.getTime() > to.getTime()) {
      res.status(400).json({ error: 'Invalid window: from must be <= to' });
      return;
    }

    const maxWindowMs = 31 * 24 * 60 * 60 * 1000;
    if (to.getTime() - from.getTime() > maxWindowMs) {
      res.status(400).json({ error: 'Window too large (max 31 days)' });
      return;
    }

    const toMysqlUtc = (d: Date): string => d.toISOString().slice(0, 19).replace('T', ' ');
    const fromStr = toMysqlUtc(from);
    const toStr = toMysqlUtc(to);

    const revenueByKindRows = await queryAll<{
      kind: string | null;
      count: number;
      total: string | null;
    }>(
      `SELECT kind, COUNT(*) as count, SUM(CAST(value AS DECIMAL(38, 0))) as total
       FROM payments
       WHERE channelId = ? AND status = 'settled' AND createdAt BETWEEN ? AND ?
       GROUP BY kind`,
      [channel.id, fromStr, toStr]
    );

    const kindCounts: Record<string, number> = {};
    const kindTotalsBaseUnits: Record<string, string> = {};
    let totalRevenueBaseUnits = 0n;
    let supportCount = 0;

    for (const row of revenueByKindRows) {
      const kind = row.kind || 'unknown';
      kindCounts[kind] = row.count || 0;
      const total = row.total || '0';
      kindTotalsBaseUnits[kind] = total;
      supportCount += row.count || 0;
      try {
        totalRevenueBaseUnits += BigInt(total);
      } catch {
        // ignore parse issues
      }
    }

    const uniqueSupportersResult = await queryOne<{ count: number }>(
      `SELECT COUNT(DISTINCT fromAddress) as count
       FROM payments
       WHERE channelId = ? AND status = 'settled' AND createdAt BETWEEN ? AND ?`,
      [channel.id, fromStr, toStr]
    );
    const uniqueSupporters = uniqueSupportersResult?.count || 0;

    const newMembersResult = await queryOne<{ count: number }>(
      `SELECT COUNT(DISTINCT fromAddress) as count
       FROM payments
       WHERE channelId = ? AND status = 'settled' AND kind = 'membership' AND createdAt BETWEEN ? AND ?`,
      [channel.id, fromStr, toStr]
    );
    const newMembers = newMembersResult?.count || 0;

    const topSupporterRows = await queryAll<{
      fromAddress: string;
      total: string | null;
      supportCount: number;
    }>(
      `SELECT fromAddress, SUM(CAST(value AS DECIMAL(38, 0))) as total, COUNT(*) as supportCount
       FROM payments
       WHERE channelId = ? AND status = 'settled' AND createdAt BETWEEN ? AND ?
       GROUP BY fromAddress
       ORDER BY total DESC
    ...
    const topSupporters = await Promise.all(
      topSupporterRows.map(async (r) => ({
        displayName: await getEffectiveDisplayName(channel.id, r.fromAddress),
        fromAddress: r.fromAddress,
        totalBaseUnits: r.total || '0',
        supportCount: r.supportCount || 0,
      }))
    );

    const newMemberRows = await queryAll<{ fromAddress: string; firstAt: string }>(
      `SELECT fromAddress, MIN(createdAt) as firstAt
       FROM payments
       WHERE channelId = ? AND status = 'settled' AND kind = 'membership' AND createdAt BETWEEN ? AND ?
       GROUP BY fromAddress
    ...
    const newMemberList = await Promise.all(
      newMemberRows.map(async (r) => ({
        displayName: await getEffectiveDisplayName(channel.id, r.fromAddress),
        fromAddress: r.fromAddress,
      }))
    );

    const topEffects = await queryAll<{ actionKey: string; count: number }>(
      `SELECT actionKey, COUNT(*) as count
       FROM payments
       WHERE channelId = ? AND status = 'settled' AND kind = 'effect'
         AND actionKey IS NOT NULL AND createdAt BETWEEN ? AND ?
       GROUP BY actionKey
       ORDER BY count DESC
       LIMIT 5`,
      [channel.id, fromStr, toStr]
    ...
    const qaHighlights = await queryAll<{
      message: string;
      tier: string;
      status: string;
      displayName: string | null;
    }>(
      `SELECT message, tier, status, displayName
       FROM qa_items
       WHERE channelId = ? AND createdAt BETWEEN ? AND ? AND status <> 'blocked'
       ORDER BY
         CASE status WHEN 'answered' THEN 0 WHEN 'showing' THEN 1 WHEN 'queued' THEN 2 ELSE 3 END,
         createdAt DESC
       LIMIT 8`,
      [channel.id, fromStr, toStr]
    );
    ...
    try {
      const recap = await generateStreamRecap({
        ...
        topSupporters,
        topEffects: topEffects.map((e) => ({ actionKey: e.actionKey, count: e.count || 0 })),
        qaHighlights: qaHighlights.map((q) => ({
      ...
      res.json({
        ok: true,
        window: { from: from.toISOString(), to: to.toISOString(), preset: preset || null },
        stats: {
          totalRevenueBaseUnits: totalRevenueBaseUnits.toString(),
          supportCount,
          uniqueSupporters,
      ...
      res.status(503).json({ error: 'AI recap unavailable', reason: (err as Error).message });
    }
  } catch (err) {
    next(err);
  }
